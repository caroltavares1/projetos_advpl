...
User Function aglutinar()
	...
	If ParamBox(aPergs, "Informe os parâmetros")
		...
		if ("QRY")->(!EOF())
			...
			AAdd(aHeader, {"C5_TIPO", cTipo, NIL})
			AAdd(aHeader, {"C5_CLIENTE", cCliente, NIL})
			AAdd(aHeader, {"C5_LOJACLI", cLojaCli, NIL})
			AAdd(aHeader, {"C5_LOJAENT", cLojaEnt, NIL})
			AAdd(aHeader, {"C5_CONDPAG", cCond, NIL})
            AAdd(aHeader, {"C5_XAGLUT", "S", NIL})
		...
		while ('QRY_SC6')->(!EOF())
            ...
            nx := soma1(nx)
            AAdd(aLine, {"C6_ITEM", nx, NIL})
            //AAdd(aLine, {"C6_NUM", cNumPed, NIL})
			AAdd(aLine, {"C6_PRODUTO", QRY_SC6 -> C6_PRODUTO, NIL})
			AAdd(aLine, {"C6_QTDVEN", QRY_SC6 -> C6_QTDVEN, NIL})
			AAdd(aLine, {"C6_PRUNIT", QRY_SC6 -> C6_PRUNIT, NIL})
			AAdd(aLine, {"C6_PRCVEN", QRY_SC6 -> C6_PRCVEN, NIL})
			AAdd(aLine, {"C6_VALOR", QRY_SC6 -> C6_VALOR, NIL})
			AAdd(aLine, {"C6_TES", QRY_SC6 -> C6_TES, NIL})
            AAdd(aLine, {"C6_XORIGEM", QRY_SC6 -> C6_NUM , NIL})
...
user function TMata410(aHeader, aItems)
